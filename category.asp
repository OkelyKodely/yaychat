...
while not rrs.eof
dim categname1
categname1 = rrs("categname")
if instr(application(categname1&"_roomlist"), session("userid") & "") > 0 then
	application(categname1&"_roomlist") = replace(application(categname1&"_roomlist"), session("userid"), "coppersink21")
end if
rrs.movenext
wend
...
sqlStr = "select p.roomid, p.categid, p.chatid as chatid, c.categname from Room p inner join Categories c on c.id = p.categid where p.categid = " & id
...
<div style="border:1px solid black;color:#ccc;width:500px">
  <A HREF="view.asp?chatid=<%=rst("chatid")%>&categid=<%=rst("categid")%>" style="color:#111;font-size:25px">Room <%=rst("roomid")%></A>
<A HREF="deleteview.asp?chatid=<%=rst("chatid")%>&categid=<%=rst("categid")%>" style="color:#ff0000;font-size:15px">Delete</A>
</div>
<%
rst.movenext
wend
